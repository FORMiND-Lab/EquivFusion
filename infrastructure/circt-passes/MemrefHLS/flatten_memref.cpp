...
namespace circt {
#define GEN_PASS_DEF_EQUIVFUSIONFLATTENMEMREFPASS
#define GEN_PASS_DEF_EQUIVFUSIONFLATTENMEMREFCALLSPASS
...
struct GlobalOpConversion : public OpConversionPattern<memref::GlobalOp> {
  ...
  matchAndRewrite(memref::GlobalOp op, OpAdaptor adaptor,
    ...
    std::optional<mlir::Attribute> initValue = op.getInitialValue();
    mlir::Attribute newInitValue = nullptr;

    if (initValue.has_value()) {
      if (llvm::isa<mlir::DenseElementsAttr>(*initValue)) {
        auto denseAttr = llvm::dyn_cast<mlir::DenseElementsAttr>(*initValue);
      ...
      } else if (llvm::isa<mlir::DenseResourceElementsAttr>(*initValue)) {
        auto denseResAttr = llvm::dyn_cast<mlir::DenseResourceElementsAttr>(*initValue);
        ShapedType resType = denseResAttr.getType();

        RankedTensorType tensorType = RankedTensorType::get({resType.getNumElements()}, type.getElementType());
        newInitValue = DenseResourceElementsAttr::get(tensorType, denseResAttr.getRawHandle());
      } else {
...
struct EquivFusionFlattenMemRefPass
    : public circt::impl::EquivFusionFlattenMemRefPassBase<EquivFusionFlattenMemRefPass> {
public:
...
struct EquivFusionFlattenMemRefCallsPass
    : public circt::impl::EquivFusionFlattenMemRefCallsPassBase<EquivFusionFlattenMemRefCallsPass> {
public:
  void runOnOperation() override {
    auto *ctx = &getContext();
    TypeConverter typeConverter;
